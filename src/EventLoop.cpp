...
namespace we
{
    ...
    Event::Event(void (*handler)(EventData), EventData data, struct timeval activation_time, unsigned long long ms_time, bool periodic) : handler(handler), data(data), ms_time(ms_time), activation_time(activation_time), periodic(periodic), id(id_generator++)
    {
    }
    ...
    bool Event::operator<(const Event &other) const
    {
        ...
        if (smaller_or_equal && (activation_time.tv_sec != other.activation_time.tv_sec || activation_time.tv_usec != other.activation_time.tv_usec))
            return true;
